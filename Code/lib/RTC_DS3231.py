# Reference: https://github.com/balance19/micropython_DS3231/blob/main/my_lib/RTC_DS3231.py 
import machine 

# Class for getting Realtime from the DS3231 in different modes.
class RTC:
    w = ["SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT"] # Weekday Name
    ...
    def DS3231_SetTime(self, NowTime=b"\x00\x00\x00\x01\x01\x01\x23"):
    ...
    # DS3231 gives data in bcd format. This has to be converted to a bcd format.
    ...
    def DS3231_ReadTime(self, mode=0):
        try:
            ...
            month = self.pre_zero(self.bcd2bin(buffer[5]))  # Just put the month value in the month variable and convert it.
            day = self.pre_zero(self.bcd2bin(buffer[4]))  # Same for the day value
